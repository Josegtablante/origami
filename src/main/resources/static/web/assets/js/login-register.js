const app = Vue.createApp({
  ...
  methods: {
    postLogin() {
      axios
        .post("/api/login", `email=${this.mail}&password=${this.password}`, {
    ...
    postRegister() {
      axios
        .post(
          "/api/clients",
          `name=${this.nameRegister}&lastName=${this.lastNameRegister}&email=${this.emailRegister}&password=${this.passwordRegister}&cellPhone=${this.cellPhoneRegister}`,
          { headers: { "content-type": "application/x-www-form-urlencoded" } }
        )
        .then((response) => {
        ...
        .catch(function (error) {
          if (
            error.response.data == "Missing data: name is empty" &&
          ...
          if (
            error.response.data == "Missing data: last name is empty" &&
            error.response.status == 403
          ) {
            Swal.fire({
              position: "center",
          ...
          if (
            error.response.data == "Missing data: email is empty" &&
            error.response.status == 403
          ) {
            Swal.fire({
              position: "center",
          ...
          if (
            error.response.data == "Missing data: password is empty" &&
            error.response.status == 403
          ) {
            Swal.fire({
              position: "center",
          ...
          if (
            error.response.data == "Missing data: cell phone is empty" &&
            error.response.status == 403
          ) {
            Swal.fire({
              position: "center",
          ...
          if (
            error.response.data == "Name already in use" &&
            error.response.status == 403
          ) {
            Swal.fire({
              position: "center",
          ...
          if (
            error.response.data == "Password to short: At least 6 characters" &&
            error.response.status == 403
          ) {
            Swal.fire({
              position: "center",
              icon: "warning",
              title:
                "Error al registrarse: La contraseña debe tener al menos 6 caracteres",
              showConfirmButton: true,
            });
          }
          if (
            error.response.data ==
              "Password to long: cannot be longer than 15 characters" &&
            error.response.status == 403
          ) {
            Swal.fire({
              position: "center",
              icon: "warning",
              title:
                "Error al registrarse: La contraseña no puede tener mas de 15 caracteres",
              showConfirmButton: true,
            });
          }
          if (
    ...
    agregarProducto(producto) {
      ...
      const agregarProductoAlCarrito = (array) => {
        if (!this.storageListID.includes(nuevoProducto.id)) {
          ...
          let productoExistente =
            this.storageCarrito[this.buscarProducto(nuevoProducto.id)];
          ...
          if (productoExistente.stock != 0) {
            ...
            productoExistente.subtotal =
              productoExistente.cantidad * productoExistente.price;
      ...
      this.actualizarSubtotal();
      Swal.fire("Hecho", "Producto agregado al carrito", "success");
    },
