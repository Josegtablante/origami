Vue.createApp({
  ...
  created() {
    ...
    axios.get(`/api/products`).then((result) => {
      ...
      axios.get("/api/clients/current").then((results) => {
        this.client = results.data;
      });

      axios.get("/api/invoicesdto").then((results) => {
              ...
              this.storageCarrito.forEach((producto) => {
                axios
                  ...
                  .then((results) => {
                    setTimeout(function () {
                      window.open(
                        `https://homebanking2.herokuapp.com/cardPayments/posnet.html?amount=${suma}`
                      );
                    }, 2000);

                    const eliminarProductoDelCarrito = (arrayOriginal) => {
                      let stockIndex = this.buscarProductoEnArray(
                        producto.id,
                        arrayOriginal
                      );
                    ...
                    this.storageCarrito.splice(
                      this.storageCarrito.indexOf(producto),
                      1
                    );
                    localStorage.setItem(
                      "cart",
                      JSON.stringify(this.storageCarrito)
                    );
                  });
              });
            })
