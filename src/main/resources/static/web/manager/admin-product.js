Vue.createApp({
  ...
  created() {
    ...
    axios.get("/api/products/" + MYPARAM).then((result) => {
    ...
    deleteProduct() {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          axios
            .patch("/api/products/status", `id=${this.product.id}`)
            .then((res) => {
              Swal.fire(
                `Producto ${this.product.name}, codigo: ${this.product.id} Eliminado`,
                "",
                "success"
              ).then(
                (res) => (window.location.href = "/web/manager/admin.html"),
                5000
              );
            })
            .catch((error) => {
          ...
          Swal.fire("Producto no eliminado", "", "error").then(
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          axios
            .patch(
              "/api/products/stock",
              `id=${this.product.id}&stock=${this.newStock}`
            )
            .then((res) => {
              Swal.fire(
                `Producto ${this.product.name}, codigo: ${this.product.id} guardado`,
                "",
                "success"
              ).then(
                (res) => (window.location.href = "/web/manager/admin.html"),
